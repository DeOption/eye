...
class Diagnosis(Base):
    ...
    latent_strabismus = Column(BOOLEAN, comment="隐斜视")
    Internal_strabismus = Column(VARCHAR(20), comment="内斜视")
    Exotropia = Column(VARCHAR(20), comment="外斜视")
    A_V = Column(VARCHAR(20), comment="A⁃V斜视")
    V = Column(VARCHAR(20), comment="垂直旋转性斜视")
    S = Column(VARCHAR(20), comment="特殊类型斜视")
    PS = Column(VARCHAR(20), comment="中枢性麻痹性斜视")
    N = Column(BOOLEAN, comment="眼球震颤")
    other = Column(BOOLEAN, comment="其他")
